...
def plot_comparison_curves(
    ...
    for y_true, y_scores, model_name in model_results:
        ...
        _add_threshold_markers(ax1, fpr, tpr, roc_thresholds, threshold_markers, color=line.get_color())

        # PR curve
        precision, recall, pr_thresholds = precision_recall_curve(flat_true, flat_scores)
        pr_auc = average_precision_score(flat_true, flat_scores)

        line = ax2.plot(recall, precision, label=f"{model_name} (AUC = {pr_auc:.3f})", lw=2)[0]
        _add_threshold_markers(ax2, recall, precision, pr_thresholds, threshold_markers, color=line.get_color())
...
def process_batch(examples: dict, detector: SpeechDetectorSileroVAD) -> dict:
    ...
    vad_probs = [
        process_audio(audio["array"], detector)
    ...
    confident_speech = [
        np.array(speech)[mask]
        for speech, mask in zip(all_speech, confident_masks)
    ]
    confident_vad_probs = [
        np.array(probs)[mask]
        for probs, mask in zip(vad_probs, confident_masks)
    ]
